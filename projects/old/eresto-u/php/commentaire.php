...
if (isset($_POST['btnAjouter'])){
    fd_controle_parametres('post', array('date', 'texte', 'note', 'btnAjouter')) 
    or fd_exit_session();
}
else if (isset($_POST['btnEnregistrer'])){ 
    fd_controle_parametres('post', array('date', 'texte', 'note', 'btnEnregistrer')) 
    or fd_exit_session();
}
else if (isset($_POST['btnUpload'])){
    fd_controle_parametres('post', array('MAX_FILE_SIZE', 'btnUpload', 'date')) 
    or fd_exit_session();
}
else{ // on vient de la page menu.php
    fd_controle_parametres('post', array('date')) or fd_exit_session();
}

// vérification de la date, si un des test des faux, la condition sera vraie, déconnexion 
if (! (isset($_POST['date']) && estEntier($_POST['date']) && estEntre($_POST['date'], 
        20200101, DATE_AUJOURDHUI))) {
        fd_exit_session();
}
...
if ($res->num_rows == 0) { // si l'étudiant n'a pas commandé de repas ce jour-là 
    $res->free();
    $conn->close();
    fd_exit_session(); // tentative de piratage ?
}
...
if (isset($_POST['btnAjouter']) || isset($_POST['btnEnregistrer'])) {
    $resultat_form1 = fdl_traitement_enregistrement($conn);
}
else {
...
if (isset($_POST['btnUpload'])) {
    $resultat_form_photo = fdl_verification_image($conn);
}
else {
...
*/
function fdl_contenu($data, $resultat_form1, $resultat_form_photo) {
    ...
    fd_afficher_resultat_soumission($resultat_form1); // n'affiche rien si pas de soumission
    ...
    echo     '<table>',
        ...
        fd_creer_select('note', 
                        array('1' => 'Nul (1/5)', '2' => 'Bof (2/5)', '3' => 'Moyen (3/5)', 
    ...
    if ($nomBtnSubmit == 'btnEnregistrer') {
        ...
        fd_afficher_resultat_soumission($resultat_form_photo); 
...
*/
function fdl_traitement_enregistrement($conn) {
    ...
    fd_verifier_texte($_POST['texte'], 'Le texte du commentaire', $err, LMAX_COTEXTE);
...
*  @return mixed   un tableau de chaînes en cas d'erreur, une chaîne non vide sinon
*/
function fdl_verification_image($conn){
