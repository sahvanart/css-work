...
if (isset($_POST['btnCommander']) && ! isset($_SESSION['user'])){
    fd_exit_session(); // ne devrait pas se produire, piratage ?
}
...
if (isset($_POST['btnCommander'])) {
    $resultat = fdl_verifier_commande($conn);
}
else {
    $resultat = FALSE;   
}
...
*/
function fdl_contenu($conn, $resultat) {

    // contrôle des paramètres reçus dans l'URL
    if (!fd_controle_parametres('GET', array(), array('jour', 'mois', 'annee'))){
    ...
    $aujourdhui = getdate(mktime(12, 0, 0, 
    substr(DATE_AUJOURDHUI, 4, 2), substr(DATE_AUJOURDHUI, 6, 2), 
    ...
    fdl_navigation_date($jour, $mois, $annee);
    ...
    fd_afficher_resultat_soumission($resultat, 
    ...
    if ($date <= DATE_AUJOURDHUI) fdl_afficher_commentaires($conn, $date, $commande);
}
...
*/
function fdl_navigation_date($jour, $mois, $annee){
    ...
    $jourSemaine = array("Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi",
    "Samedi");
    ...
    echo '<h2>',
        ...
        fd_creer_liste_nombre('annee', 2020, 2021, 1, $annee);
...
*/
function fdl_afficher_plat($i, $inForm = false) {
    ...
    $categorie2name = array('entree' => 'radEntree', 
                            'viande' => 'radPlat', 
                            'poisson' => 'radPlat',
                            'dessert' => 'radDessert', 
                            'fromage' => 'radDessert', 
    ...
    echo '<input id="', $id, '" name="', $name, '" type="', $type, '" value="', $i['plID'], '"', 
...
*/
function fdl_afficher_divers($i) {
...
*/
function fdl_verifier_commande($conn) {
...
*/
function fdl_afficher_commentaires($conn, $date, $commOK) {
    ...
    $nb = count($comments);
    if ($nb == 0) echo '<p>Pas de commentaires pour ce menu.</p>'; return;
