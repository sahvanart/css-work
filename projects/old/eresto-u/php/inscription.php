...
if (isset($_POST['btnInscription'])) {
    $erreurs = fdl_traitement_inscription();
}
else{
    $erreurs = FALSE;
}
...
function fdl_contenu($erreurs) {
    ...
    fd_afficher_resultat_soumission($erreurs, 'Les erreurs suivantes ont été relevées lors de votre inscription');
    ...
    fd_creer_liste_nombre('annee', $anneeCourante-NB_ANNEE_DATE_NAISSANCE+1, $anneeCourante, -1, $annee);
    echo        '</td>', 
            '</tr>',
            '<tr>',
...
*/
function fdl_traitement_inscription() {
    ...
    de la fonction fd_exit_session() */

    if( !fd_controle_parametres('post', 
            array('login', 'nom', 'prenom', 'jour', 'mois', 'annee', 'passe1', 'passe2',
    ...
    fd_verifier_texte($nom, 'Le nom', $erreurs, LMAX_NOM);
    fd_verifier_texte($prenom, 'Le prénom', $erreurs, LMAX_PRENOM);
    ...
    if (! (estEntier($_POST['annee']) 
            && estEntre($_POST['annee'], $anneeCourante-NB_ANNEE_DATE_NAISSANCE+1,
            $anneeCourante))){
        fd_exit_session(); 
    }
