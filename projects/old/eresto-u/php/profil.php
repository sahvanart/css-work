...
if (isset($_POST['btnSave'])){
    fd_controle_parametres('post', array('nom', 'prenom', 'btnSave')) 
    or fd_exit_session();
}
else if (isset($_POST['btnEnr'])){ 
    fd_controle_parametres('post', array('pass1', 'pass2', 'btnEnr')) 
    or fd_exit_session();
}
...
function sll_affich_id($resultat_form_id, $statistiques){
    ...
    fd_afficher_resultat_soumission($resultat_form_id);   
...
function sll_affich_mdp($resultat_form_pass){
    ...
    '<section>',
        ...
        fd_afficher_resultat_soumission($resultat_form_pass);
...
function sll_changer_id($conn) {
    ...
    fd_verifier_texte($nom, 'Le nom', $resultat_form_id, LMAX_NOM);
    fd_verifier_texte($prenom, 'Le prénom', $resultat_form_id, LMAX_PRENOM);
